...
(async () => {
  ...
  for (const mobile of [false, true]) {
    ...
    if(mobile) await page.screenshot({path:require('path').join(__dirname, 'mobile-after.png')});
  ...
  await browser.close();
  fs.writeFileSync(require('path').join(__dirname, 'browser-results-after.json'),JSON.stringify(results,null,2));
