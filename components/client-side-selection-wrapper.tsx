...
export default function ClientSideSelectionWrapper({
  featuredShows,
  title = 'COMING UP',
}: ClientSideSelectionWrapperProps) {
        ...
        <CardContent className='p-0 relative h-full flex flex-col'>
          <div className='aspect-square w-full relative'>
            <Image
              ...
              alt={selectedShow.title || 'Selected Show'}
              fill
            ...
            <>
              <div className='absolute top-1/2 left-4 transform -translate-y-1/2'>
                <Button
                  ...
                  onClick={() => {
                    const prevIndex =
                      (selectedIndex - 1 + featuredShows.length) % featuredShows.length;
      ...
      {featuredShows.length > 0 && (
        <div className='grid grid-cols-5 gap-2 mt-6'>
          {featuredShows.map((show, index) => (
            <button
              key={index}
              onClick={() => handleSelectShow(show, index)}
