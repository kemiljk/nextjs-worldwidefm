...
interface TransformedHeroItem extends CosmicItem {
  key: string;
  url: string;
}
...
const HeroItem = ({ item, isPriority }: { item: TransformedHeroItem; isPriority: boolean }) => {
  ...
  const isCurrentShow = selectedShow?.key === item.key;
  ...
  const handlePlayPause = (e: React.MouseEvent) => {
    ...
    // Use the same logic as ShowCard - pass the item directly to playShow
    if (isCurrentShow) {
      if (isCurrentlyPlaying) {
        pauseShow();
      } else {
        playShow(item as any);
      }
    } else {
      playShow(item as any);
    }
  ...
  const shouldShowPlayButton = item.type === 'episodes' && item.metadata?.player;
