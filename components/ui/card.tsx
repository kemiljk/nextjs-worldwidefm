...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => <div ref={ref} className={cn("rounded-none border border-black dark:border-white bg-white dark:bg-black text-card-foreground", className)} {...props} />);
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(({ className, ...props }, ref) => <h3 ref={ref} className={cn("text-h8 font-display uppercase font-normal text-almostblack leading-none tracking-tight", className)} {...props} />);
