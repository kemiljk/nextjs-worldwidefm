...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'z-60 flex h-auto w-auto items-center gap-2 rounded-xl border-almostblack bg-almostblack px-3 py-1.5 font-mono uppercase text-[12px] text-white placeholder:text-muted-foreground focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 cursor-pointer dark:bg-white dark:text-almostblack',
      className
    )}
    {...props}
  >
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        ...
        position === 'popper' &&
          'data-[side=bottom]:translate-y data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1',
        className
      )}
      position={position}
      {...props}
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      'relative flex cursor-pointer select-none items-center py-1.5 text-[12px] px-3 font-mono uppercase outline-hidden rounded-xl hover:bg-black hover:text-white focus:bg-accent focus:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 dark:hover:bg-white dark:hover:text-almostblack',
      className
    )}
    {...props}
  >
