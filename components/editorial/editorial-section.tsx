            ...
            return (
              <Link href={`/editorial/${post.slug}`} key={post.slug} className="group flex gap-6 items-start hover:bg-white/50 dark:hover:bg-white/5 p-4 -mx-4 rounded-none transition-colors">
                ...
                <div className="flex-grow pt-2">
                  <div className="text-[12px] leading-none uppercase tracking-wider text-muted-foreground mb-2">{formattedDate}</div>
                  <h3 className="text-xl leading-tight transition-colors mb-2">{post.title}</h3>
                  ...
                  {post.metadata.author && <div className="text-[10px] leading-none uppercase tracking-wider text-muted-foreground mt-3">By {typeof post.metadata.author === "string" ? post.metadata.author : post.metadata.author.title || "Unknown"}</div>}
                </div>
              </Link>
            );
          })}
