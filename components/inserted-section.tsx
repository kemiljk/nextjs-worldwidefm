...
const SectionItemCard: React.FC<{ item: CosmicItem }> = ({ item }) => {
  const imageUrl = item.metadata?.image?.imgix_url || item.metadata?.image?.url || "/image-placeholder.svg";
  ...
  return (
    <div className="border border-almostblack dark:border-white overflow-hidden">
      ...
      <div className="p-4">
        ...
        {subtitle && <p className="font-body text-sm text-almostblack dark:text-white mb-2 line-clamp-1">{subtitle}</p>}
        {description && <p className="font-body text-sm text-almostblack dark:text-white mb-2 line-clamp-2">{description}</p>}
