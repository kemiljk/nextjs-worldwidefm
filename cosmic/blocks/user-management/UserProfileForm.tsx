...
export function UserProfileForm({ user }: UserProfileFormProps) {
  ...
  const [avatarPreview, setAvatarPreview] = useState<string | null>(user.metadata.avatar?.imgix_url || null);
    ...
    <form action={handleSubmit} className="w-full max-w-md space-y-6">
      <div className="flex flex-col items-center gap-4">
        ...
        <div>
          ...
          <label htmlFor="avatar" className="cursor-pointer rounded-none bg-zinc-100 px-4 py-2 text-sm transition-colors hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700">
            Change Avatar
          </label>
        </div>
      </div>
      ...
      <div>
        ...
        <Input type="text" id="firstName" name="firstName" defaultValue={user.metadata.first_name} required />
      </div>

      <div>
        <Label htmlFor="lastName">Last Name</Label>
        <Input type="text" id="lastName" name="lastName" defaultValue={user.metadata.last_name} required />
      </div>

      <div>
        <Label htmlFor="email">Email</Label>
        <Input type="email" id="email" name="email" defaultValue={user.metadata.email} required />
        {!user.metadata.email_verified && <p className="mt-1 text-sm text-amber-600">Email not verified</p>}
      </div>

      {message && <div className={`text-center ${message.startsWith("Error") ? "text-red-500" : "text-green-500"}`}>{message}</div>}
