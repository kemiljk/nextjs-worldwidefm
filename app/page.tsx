...
import { getScheduleData, getEditorialContent, getVideos } from "@/lib/actions";
...
export default async function Home() {
  ...
  const limitedUpcomingShows = upcomingShows.slice(0, 4);
  ...
  return (
    <div className="min-h-screen -mx-4 md:-mx-8 lg:-mx-16">
      ...
      <div>
        ...
        <section className="px-4 md:px-8 lg:px-24 py-16 border-t border-b border-bronze-500 bg-bronze-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {limitedUpcomingShows.map((show, index) => (
              <Link key={show.id} href={`/schedule/${show.slug}`}>
        ...
        <section className="px-4 md:px-8 lg:px-24 py-16 border-t border-b border-green-500 bg-green-500">
          <div className="flex items-center justify-between mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {limitedUpcomingShows.map((show, index) => (
              <Link key={show.id} href={`/archive/${show.slug}`}>
                <Card className="overflow-hidden border-none hover:shadow-lg transition-shadow">
                  <CardContent className="p-0">
                    <div className="relative aspect-square">
                      <Image src={show.image || "/placeholder.svg"} alt={show.title} fill className="object-cover" />
        ...
        {/* Video Section */}
        <VideoSection videos={videos} className="px-4 md:px-8 lg:px-24 py-16 border-t border-b border-crimson-500 bg-crimson-500" />

        {/* Editorial section */}
        <EditorialSection posts={limitedPosts} className="px-4 md:px-8 lg:px-24 py-16 border-t border-b border-sky-500 bg-sky-300" isHomepage={true} />
      </div>
