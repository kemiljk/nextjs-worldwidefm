    ...
    return (
      <div className='flex flex-col items-center justify-center min-h-[60vh] text-center'>
        ...
        <p className='text-lg text-muted-foreground mb-6'>
          Sorry, we couldn't find a show for this link. It may have been removed or does not exist.
        </p>
        <Link
          href='/shows'
          ...
          <div className='bg-white dark:bg-gray-900 p-6 rounded-lg'>
            <div className='space-y-6'>
              <div>
                ...
                {(show as any).broadcast_time && (
                  <dd className='mt-1 text-sm text-muted-foreground'>
                    Broadcast: {(show as any).broadcast_time}
                  </dd>
                )}
                {(show as any).duration && (
                  <dd className='mt-1 text-sm text-muted-foreground'>
                    Duration: {(show as any).duration}
                  </dd>
                )}
              </div>
              {!isLive && !isRadioCult && (
            ...
            {((show as any).enhanced_hosts?.length > 0 ||
              show.hosts?.length > 0 ||
              (isRadioCult && (show as any).artists?.length > 0)) && (
              <div className='mt-8'>
                ...
                <div className='flex flex-wrap gap-2'>
                  ...
                  {(show as any).enhanced_hosts?.length > 0
                    ? (show as any).enhanced_hosts.map((host: any) => (
                        <HostLink
                          key={host.id || host.key}
                          host={host}
                          className='bg-bronze-50 dark:bg-bronze-950 hover:bg-bronze-300 dark:hover:bg-bronze-900 text-bronze-900 dark:text-bronze-100 px-3 py-1 rounded-full text-sm transition-colors border border-bronze-300 dark:border-bronze-600'
                        />
                      ))
                    : show.hosts?.map((host) => (
                        <HostLink
                ...
                <div className='flex flex-wrap gap-2'>
                  {(show as any).enhanced_genres?.map((genre: any) => (
                    <span
                      ...
                      className='bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full text-sm uppercase'
                    >
              ...
              <div className='mt-8'>
                ...
                <div className='flex flex-wrap gap-2'>
                  {(show as any).takeovers.map((takeover: any) => (
                    <span
                      key={takeover.id}
                      className='bg-purple-100 dark:bg-purple-900 px-3 py-1 rounded-full text-sm'
                    >
                      {takeover.title}
                    </span>
                  ))}
                    ...
                    <div className='aspect-square relative'>
                      ...
                      <div className='absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end'>
                        <div className='p-4 w-full'>
                          ...
                          {relatedShow.tags && relatedShow.tags.length > 0 && (
                            <p className='text-white/70 text-sm mt-1'>{relatedShow.tags[0].name}</p>
                          )}
                        </div>
                      </div>
