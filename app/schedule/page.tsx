          ...
          {scheduleItems.length > 0 ? (
            <div className="divide-y divide-gray-100">
              {scheduleItems.map((show, index) => (
                <Link href={`/shows/${show.slug}`} key={show.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors group">
                  {/* Show thumbnail */}
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-medium text-gray-900 group-hover:text-brand-orange transition-colors">{show.title}</h3>
                    {show.metadata?.subtitle && <p className="text-sm text-gray-500 mt-1 line-clamp-1">{show.metadata.subtitle}</p>}
