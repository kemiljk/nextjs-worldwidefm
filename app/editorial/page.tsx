...
import { getEditorialHomepage, getArticles, getWatchAndListenItems, getMoods } from "@/lib/cosmic-service";
...
export default async function EditorialPage() {
  ...
  const otherWatchListenItems = watchListenItems.filter((item) => item.slug !== "album-of-the-week" && item.slug !== "events" && item.slug !== "video");
  ...
  return (
    <div className="min-h-screen bg-sky-700 dark:bg-sky-900">
      <div className="container mx-auto pt-32 pb-32">
        ...
        {/* Moods Section - Enhanced with MoodCard component */}
        {moods.length > 0 && (
          <div className="mb-16">
            ...
            {/* Show more moods in a scrolling container if there are more than 10 */}
            {moods.length > 10 && (
              <div className="mt-6">
                <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar">
                  {moods.slice(10).map((mood) => (
                    <MoodCard key={mood.id} mood={mood} />
                  ))}
                </div>
              </div>
              ...
              {albumOfTheWeek && (
                <Card className="overflow-hidden border-none shadow-md">
                  <CardContent className="p-0 relative">
                    ...
                    <Image src={albumOfTheWeek.metadata.image.imgix_url || "/placeholder.svg"} alt={albumOfTheWeek.title} width={400} height={400} className="w-full aspect-square object-cover" />
                    <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-2xl p-4 text-white">
                      <p className="text-sm line-clamp-2 mb-2">{albumOfTheWeek.metadata.description}</p>
                      <Link href={albumOfTheWeek.metadata.link} className="text-brand-orange text-sm font-medium flex items-center">
                ...
                <Card className="overflow-hidden border-none shadow-md">
                  <CardContent className="p-0 relative overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-2xl p-4 text-white">
                      ...
                      <Link href={events.metadata.link} className="text-brand-orange text-sm font-medium flex items-center">
              ...
              {video && (
                <Card className="overflow-hidden border-none shadow-md">
                  <CardContent className="p-0 relative">
                    ...
                    <Image src={video.metadata.image.imgix_url || "/placeholder.svg"} alt={video.title} width={400} height={400} className="w-full aspect-square object-cover" />
                    <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-2xl p-4 text-white">
                      <p className="text-sm line-clamp-2 mb-2">{video.metadata.description}</p>
                      <Link href={video.metadata.link} className="text-brand-orange text-sm font-medium flex items-center">
        ...
        {articles.length > 0 && (
          <div>
                ...
                <Card className="overflow-hidden border-none shadow-md">
                  <CardContent className="p-0 flex flex-col md:flex-row">
                    <div className="md:w-1/2 relative">
                      <Image src={articles[0].metadata.image?.imgix_url || "/placeholder.svg"} alt={articles[0].title} width={800} height={500} className="w-full aspect-video md:h-full object-cover" />
                    </div>
            ...
            <div className="mt-8 text-center">
              <Link href="/articles" className="inline-block px-6 py-3 bg-brand-orange text-white rounded-lg hover:bg-brand-orange/90 transition-colors">
                View All Articles
              </Link>
            </div>
          </div>
