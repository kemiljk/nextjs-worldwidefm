...
export function AddShowForm() {
  ...
  // Fetch artists, genres, and locations when component mounts
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        // Fetch data in parallel
        const [artistList, genresResult, locationsList] = await Promise.all([getArtists(), fetchGenres(), getCosmicLocations()]);
                  ...
                  {artistInput && (
                    <CommandList>
                      {artists
                        ...
                        .map((artist) => (
                          <CommandItem key={artist.id} value={artist.id} onSelect={() => handleArtistSelect(artist)} className="cursor-pointer">
                            {artist.name}
                          </CommandItem>
                        ))}
                      {artists.filter((artist) => artist.name.toLowerCase().includes(artistInput.toLowerCase())).length === 0 && <CommandEmpty>No artists found matching "{artistInput}"</CommandEmpty>}
                    </CommandList>
                  )}
                </Command>
                    ...
                    {selectedGenreInput && (
                      <CommandList>
                        {genres
                          ...
                          .map((genre) => (
                            <CommandItem key={genre.id} value={genre.id} onSelect={() => handleGenreSelect(genre.id)} className="cursor-pointer">
                              {genre.title}
                            </CommandItem>
                          ))}
                        {genres.filter((genre) => !selectedGenres.includes(genre.id) && genre.title.toLowerCase().includes(selectedGenreInput.toLowerCase())).length === 0 && <CommandEmpty>No genres found matching "{selectedGenreInput}"</CommandEmpty>}
                      </CommandList>
                    )}
                  </Command>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {selectedGenres.map((genreId) => (
                        <Badge key={genreId} variant="secondary" className="flex items-center gap-1">
                          ...
                          <X className="h-3 w-3 cursor-pointer" onClick={() => handleGenreRemove(genreId)} />
                        </Badge>
                      ))}
                    </div>
                  )}
                </div>
                <FormDescription>Select genres to categorize this show</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
                  ...
                  {locationInput && (
                    <CommandList>
                      {locations
                        ...
                        .map((location) => (
                          <CommandItem key={location.slug} value={location.slug} onSelect={() => handleLocationSelect(location)} className="cursor-pointer">
                            {location.title}
                          </CommandItem>
                        ))}
                      {locations.filter((location) => location.title.toLowerCase().includes(locationInput.toLowerCase())).length === 0 && <CommandEmpty>No locations found matching "{locationInput}"</CommandEmpty>}
                    </CommandList>
                  )}
                </Command>
