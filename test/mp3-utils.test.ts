...
const TAGGED_MP3 = Buffer.from([
  0x49, 0x44, 0x33, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xfb, 0xe0, 0x40, 0x00, 0x00,
]);

const FF_DOMINANT_AFTER_ID3 = Buffer.from([
  0x49, 0x44, 0x33, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00,
]);

const ZERO_DOMINANT = Buffer.from([
  0x49, 0x44, 0x33, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
]);
...
describe('inspectMp3Structure', () => {
  ...
  it('reports mixed padding when neither FF nor 00 dominates', () => {
    const mixed = Buffer.from([
      0x49, 0x44, 0x33, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff,
      0x00,
    ]);
...
describe('writeMp3Id3v23Metadata', () => {
  it('writes a TIT2 frame before tagless audio data', () => {
    const patched = writeMp3Id3v23Metadata(TAGLESS_MP3, {
      title: 'raw20250312 Morning Show',
    });

    expect(patched.slice(0, 3).toString('ascii')).toBe('ID3');
    expect(patched.slice(10, 14).toString('ascii')).toBe('TIT2');
    expect(inspectMp3Structure(patched).hasMpegFrameSync).toBe(true);
  });

  it('replaces an existing ID3v2 tag and keeps the MPEG audio', () => {
    const patched = writeMp3Id3v23Metadata(TAGGED_MP3, {
      title: 'raw20250312 Morning Show',
      artist: 'Worldwide FM',
    });
    ...
    expect(patched.slice(0, 3).toString('ascii')).toBe('ID3');
    expect(patched.slice(10, 14).toString('ascii')).toBe('TIT2');
    expect(patched.includes(Buffer.from('TPE1', 'ascii'))).toBe(true);
    expect(inspectMp3Structure(patched).hasMpegFrameSync).toBe(true);
  });
});
