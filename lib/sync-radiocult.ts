    ...
    const response = await cosmic.objects
      .find({
        ...
        $or: [
          ...
          { 'metadata.radiocult_synced': false },
        ],
      })
...
async function syncShowToRadioCult(show: RadioShowObject) {
  try {
    ...
    const startTimeISO = broadcastToISOString(
      customMetadata.broadcast_date,
      customMetadata.broadcast_time,
      customMetadata.broadcast_date_old
    );
    if (!startTimeISO) {
      console.error('Invalid broadcast date/time for show:', show.title);
      return { success: false, error: 'Invalid broadcast date/time' };
    }
    ...
    const eventData = {
      ...
      tags:
        customMetadata.radiocult_tag_ids || customMetadata.genres?.map((g: any) => g.title) || [],
    };
    ...
    if (!response.ok) {
      ...
      console.error(
        `Failed to ${method === 'POST' ? 'create' : 'update'} RadioCult event:`,
        errorText
      );
    ...
    console.log(
      `Successfully ${method === 'POST' ? 'created' : 'updated'} RadioCult event ${eventId} for show ${show.id}`
    );
...
async function createOrUpdateEpisodeObject(
  show: RadioShowObject,
  eventId: string,
  customMetadata: any
) {
  try {
