...
let citiesCache: { [key: string]: Location[] } = {};
...
export async function getCities(countryCode?: string, name?: string): Promise<Location[]> {
  ...
  if (citiesCache[countryCode]) {
    ...
    return cities.filter((city: Location) => city.name.toLowerCase().includes(searchTerm) || (city.region && city.region.toLowerCase().includes(searchTerm)));
  }
  ...
  try {
    ...
    if (name) {
      const searchTerm = name.toLowerCase();
      return fallbackCities.filter((city: Location) => city.name.toLowerCase().includes(searchTerm));
    }
