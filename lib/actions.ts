...
export async function getPostsWithFilters({
  ...
  try {
    console.log('getPostsWithFilters called with:', {
      limit,
      offset,
      searchTerm,
      categories,
    ...
    // Add category filter - use ID like Shows page does for genres
    ...
    if (postType) {
      // Convert postType to the display value for Cosmic query
      const typeValue =
...
export async function getPostBySlug(
  ...
  try {
    const query = { type: 'posts', slug: slug };
    const props = ['id,slug,title,metadata,modified_at,created_at,status'];
...
export async function getRelatedPosts(post: PostObject): Promise<PostObject[]> {
  try {
    ...
    // First try to use tags if available, otherwise fall back to categories
    ...
    if (post.metadata.tags && Array.isArray(post.metadata.tags) && post.metadata.tags.length > 0) {
      ...
      console.log('Using tags for related posts:', searchTerms);
    }
    // Fall back to categories if no tags
      ...
      searchTerms = post.metadata.categories
        .map(cat => {
          ...
          if (cat && typeof cat === 'object') {
            // Prefer slug for better matching
      ...
      console.log('Using categories as fallback for related posts:', searchTerms);
    }

    if (searchTerms.length === 0) {
      console.log('No valid tags or categories found for post:', post.slug);
      return [];
    }
    ...
    if (searchField === 'metadata.tags') {
      // For tags, we can use direct array matching
      query['metadata.tags'] = { $in: searchTerms };
    } else {
      // For categories, we need to match against the slug field
      query['metadata.categories.slug'] = { $in: searchTerms };
    }

    // Fetch posts that share at least one tag/category with the current post
    const relatedPosts = await cosmic.objects
      .find(query)
      .props('id,title,slug,type,created_at,metadata')
      .limit(5) // Get more to ensure we have enough after filtering
      .depth(2);
    ...
    console.log(`Found ${filteredPosts.length} related posts for post:`, post.slug);
